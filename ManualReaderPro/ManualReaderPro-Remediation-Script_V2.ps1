...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        MD5                 = "287de08218ea23f7e795da3caf525bb6"
        Authentihash        = "feaadae84cbba1e89caa18e019f955d8ec8b85d7c99008f5ea339079647c1b62"
    }

    Name = "ManualReaderPro"

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Processes
    # ----------------------------------------------------------------------------
    # Processes written/spawned by the installer that should be terminated

    Processes = @(
        "manualreaderpro"       # Primary installer PE
        "ManualReaderPro"       # Installed application binary
        "node"                  # Bundled Node.js runtime dropped to AppData\Local\Programs\ManualReaderPro\node\
    )

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Services
    # ----------------------------------------------------------------------------
    # No persistent Windows services were observed; placeholder retained.
    ...
    Certificates = @{
        ...
        ProtectedKeywords = @(
            ...
            "Comodo",
            "GlobalSign",
            "Sectigo"       # Sectigo root/intermediate chains are legitimate; only the leaf is revoked
        )
    ...
    # ----------------------------------------------------------------------------
    # VT confirmed: schtasks /Create /tn "697ea700-bd22-4670-9bf0-638b328e68dc"
    # Task XML was staged from %TEMP%\is-H5D3J.tmp\task.xml
    # Pattern targets the known GUID and any UUID-format task name spawned from Temp

    TaskPatterns = @(
        "697ea700-bd22-4670-9bf0-638b328e68dc"   # Observed hardcoded GUID task name
        "ManualReaderPro"                         # Named task variant
        "manualreaderpro"
    )
    ...
    # ----------------------------------------------------------------------------
    # No explicit Run key persistence was confirmed in VT behavioral data.
    # Uninstall key exists at HKCU; remediation handled under RegistryHKUPatterns.
    ...
    RegisteredAppPatterns = @(
        "ManualReaderPro"
        "ManualReaderPro version 1.0.0"
    )

    # Exact user-profile install path observed during analysis
    UserPaths = @(
        "%LOCALAPPDATA%\Programs\ManualReaderPro"
    )
    ...
    # Download/dropper staging patterns (Temp artifacts)
    DownloadPatterns = @(
        "manualreaderpro.exe"
        "manualreaderpro.tmp"
        "manualreaderpro (1).exe"
        "70k00g.exe"                # Alternate filename observed in VT names list
        "\$RDT9RL3.exe"            # Recycle Bin staging name
    )

    # User-profile system paths written by the installer
    SystemPaths = @(
        "%LOCALAPPDATA%\Programs\ManualReaderPro"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\ManualReaderPro.exe"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\unins000.exe"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\unins000.dat"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\node"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\node\node.exe"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\runtimes"
        "%LOCALAPPDATA%\Programs\ManualReaderPro\runtimes\win-x86\native\WebView2Loader.dll"
        "%APPDATA%\Microsoft\Windows\Start Menu\Programs\ManualReaderPro.lnk"
    )
    ...
    # HKLM keys written or queried for suspicious purposes
    RegistryHKLM = @(
        "SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\manualreaderpro.exe"
        "SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\manualreaderpro.tmp"
        "SOFTWARE\WOW6432Node\Microsoft\CTF\Compatibility\manualreaderpro.tmp"
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ManualReaderPro_is1"
    )

    # HKU / HKCU patterns for per-user hive cleanup
    RegistryHKUPatterns = @(
        "Software\Microsoft\Windows\CurrentVersion\Uninstall\ManualReaderPro_is1"
        "Software\Microsoft\RestartManager\Session0000"  # Created and deleted by installer; flag residue
    )
    ...
    # Start Menu LNK written to user profile
    ...
    # ----------------------------------------------------------------------------
    # Mixpanel telemetry endpoints contacted during execution (analytics/C2 beacon)
    ...
    # ----------------------------------------------------------------------------
    # DNS queries observed: install.manualreaderproweb.com, view.manualreaderproweb.com,
    # api.mixpanel.com, api.mixpnl.com (typosquat), manualreaderpro.com,
