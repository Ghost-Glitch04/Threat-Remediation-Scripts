...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "WebCompanion/Lavasoft Adware"
        FirstSeen = "2025-12-01"
        Severity = "HIGH"
        Description = "Web Companion adware/PUP by Lavasoft (7270356 Canada Inc.) - Browser hijacker with trojan characteristics, anti-debugging capabilities, and WMI-based system enumeration"
        CVSS = "7.8"
        ThreatIntelSource = "VirusTotal - 20/76 malicious detections"
        ThreatCategory = "trojan.webcompanion/msil"
        FileHashes = @{
            MD5 = "84cfe2c1c75670deccc2287cc579ed2e"
            SHA1 = "a604bb514b984eaa600666e43323bc4d51c39730"
            SHA256 = "7a115b1929a467815f9aeddd8c1add59dc9618fa8b400e655481f51ad18e58d9"
        }
    }
    ...
    Processes = @(
        ...
        "WebCompanion",
        "lvatfv",
        "14EQN3NTTX7QE10J"  # Observed temporary execution name
    )
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "56F38BE28047D9123F6DC63C0F26D557F815011F",  # 7270356 Canada Inc. code signing cert
            "A8D66DE587A6065376706BE970D929F3BAC3B28C",   # lavasoft.com SSL cert (WR1)
            "BEF8E057CA64157A248F64082223ADBB79320BC5",   # lavasoft.com SSL cert (WE1)
            "FD7453FB0D40499D0DAAD35C5C7BC15412519423"    # flwadw.com SSL cert
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "06ECC76A7973D9D7D97F3318994476BF",           # 7270356 Canada Inc.
            "009AEAAE6770DA302613FC3C92F2CA6327",         # lavasoft.com (WR1)
            "6C8593A3D494B9600E1CA03F9A349256",           # lavasoft.com (WE1)
            "00C75D2D9EA670989B134A5FCA8CDE9A2B"          # flwadw.com
        )
        ...
        ProtectedKeywords = @(
            ...
            "Comodo",
            "GlobalSign",
            "Entrust"  # Legitimate CA used by malware, but protect legitimate uses
        )
    ...
    UserPaths = @(
        "AppData\Local\Temp\WC\*",
        "AppData\Local\Temp\WcInstaller.log",
        "AppData\Roaming\Lavasoft\*",
        "Downloads\WCInstaller.exe",
        "Downloads\WebCompanion.exe"
    )
    ...
    SystemPaths = @(
        ...
        "$env:ProgramData\Lavasoft\Web Companion",
        "$env:ProgramData\Lavasoft\Web Companion\Options",
        "$env:ProgramFiles\Lavasoft",
        "$env:ProgramFiles\Web Companion",
        "${env:ProgramFiles(x86)}\Lavasoft",
        "${env:ProgramFiles(x86)}\Web Companion",
        "$env:TEMP\WC",
        "$env:TEMP\WcInstaller.log",
        "$env:TEMP\14EQN3NTTX7QE10J.exe"
    )
    ...
    RegistryHKLM = @(
        ...
        "SOFTWARE\WOW6432Node\Lavasoft",
        "SOFTWARE\WOW6432Node\Lavasoft\Web Companion",
        "SOFTWARE\Wow6432Node\Microsoft\Tracing\*WCInstaller*",
        "SOFTWARE\Wow6432Node\Microsoft\Tracing\*WebCompanion*",
        "SOFTWARE\Wow6432Node\Microsoft\Tracing\14EQN3NTTX7QE10J_RASAPI32",
        "SOFTWARE\Wow6432Node\Microsoft\Tracing\14EQN3NTTX7QE10J_RASMANCS",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*Lavasoft*",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*WebCompanion*",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*Web Companion*"
    )
    ...
    RegistryHKUPatterns = @(
        "Software\Lavasoft",
        "Software\Lavasoft\Web Companion",
        "Software\Microsoft\Windows\CurrentVersion\Run\*WebCompanion*",
        "Software\Microsoft\Windows\CurrentVersion\Run\*Lavasoft*",
        "Software\Microsoft\SystemCertificates\Root\Certificates\0174E68C97DDF1E0EEEA415EA336A163D2B61AFD"  # GoProxy cert installed by malware
    )
    ...
    BrowserStartMenuPatterns = @(
        ...
        "*lavasoft.com*",
        "*rt.webcompanion.com*",
        "*staging-webcompanion.lavasoft.net*"
    )
    ...
    # Command & Control / Malicious Domains (for host file blocking or firewall rules)
    MaliciousDomains = @(
        ...
        "flow.lavasoft.com",
        "staging-webcompanion.lavasoft.net",
        "wcdownloader-qa.lavasoft.com"
    )

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Mutexes
    # ----------------------------------------------------------------------------

    # Known malware mutexes (for detection/removal)
    Mutexes = @(
        "AdwareWCInstaller",
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - WMI Queries (Detection Signatures)
    # ----------------------------------------------------------------------------

    # WMI queries used by malware for system enumeration
    WMIQueries = @(
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Behavior Tags
    # ----------------------------------------------------------------------------

    BehaviorTags = @(
        "DETECT_DEBUG_ENVIRONMENT",  # Anti-debugging techniques
        "CALLS_WMI",                 # WMI-based enumeration
        "CHECKS_NETWORK_ADAPTERS",   # Network reconnaissance
        "OBFUSCATED",                # Code obfuscation
        "LONG_SLEEPS",               # Sandbox evasion
        "SIGNED"                     # Code-signed malware
    )
}
