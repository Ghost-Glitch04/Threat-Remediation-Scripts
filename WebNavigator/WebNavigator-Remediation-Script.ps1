...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "WebNavigatorBrowser"
        FirstSeen = "2020-04-21"
        Severity = "HIGH"
        Description = "WebNavigatorBrowser adware/PUA - Browser hijacker with " +
                      "revoked certificate signed by Better Cloud Solutions LTD. " +
                      "Installs browser extensions, modifies proxy settings, " +
                      "and establishes persistence via scheduled tasks."
        CVSS = "7.5"
        ThreatIntelSource = "VirusTotal - 30-45/76 detections across samples"
        AlternateNames = @("WebNavigator", "WebNav", "WebNavigatorBrowser")
    }
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "EA955FA1458809A912B53EE82A90658CFC6EB30D",  # Better Cloud Solutions
            "69627904BC889EC7CDBE298E1344437768077045137BC6A6DFEDD0295300C9FE",
            "83FDE1BA76FEF55291B50D6861906DAA45B58CB5",  # GlobalSign TSA
            "B1EE85FA550EFDCF1CCA641D9743EC0A"
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "00 F2 03 B9 1A 7C 51 72 71 E8 8B 08 15 A5 CA 06 21",
            "00F203B91A7C517271E88B0815A5CA0621",
            "F2 03 B9 1A 7C 51 72 71 E8 8B 08 15 A5 CA 06 21",
            "F203B91A7C517271E88B0815A5CA0621",
            "11 21 B4 55 35 1E BB 1A B2 4F 97 EF 07 FE 2A B3 0B 8A",
            "1121B455351EBB1AB24F97EF07FE2AB30B8A"
        )
    ...
    TaskPatterns = @(
        "*WebNavigatorBrowser*",
        "*BetterCloudSolutions*",
        "WebNavigatorBrowser-StartAtLogin",
        "*Better Cloud Solutions*"
    )
    ...
    UserPaths = @(
        "$env:LOCALAPPDATA\WebNavigatorBrowser",
        "$env:APPDATA\WebNavigatorBrowser",
        "$env:TEMP\Chrome-bin",
        "$env:TEMP\webnavigatorbrowser_installer.log"
    )
    ...
    # Download folder patterns (specific to WebNavigatorBrowser)
    DownloadPatterns = @(
        "*webnavigatorbrowser*",
        "*WebNavigatorBrowser*",
        "*notification_helper*",
        "setup.exe",
        "*webnavigatorbrowser.7z*",
        "*Chrome-bin*"
    )
    ...
    SystemPaths = @(
        "$env:LOCALAPPDATA\WebNavigatorBrowser",
        "$env:LOCALAPPDATA\Temp\Chrome-bin",
        "$env:ProgramFiles\WebNavigatorBrowser",
        "$env:ProgramFiles\Better Cloud Solutions",
        "${env:ProgramFiles(x86)}\WebNavigatorBrowser",
        "${env:ProgramFiles(x86)}\Better Cloud Solutions",
        "$env:ProgramData\WebNavigatorBrowser",
        "$env:TEMP\webnavigatorbrowser*",
        "$env:TEMP\Chrome-bin"
    )
    ...
    RegistryHKLM = @(
        "HKLM:\SOFTWARE\WebNavigatorBrowser",
        "HKLM:\SOFTWARE\WOW6432Node\WebNavigatorBrowser",
        "HKLM:\SOFTWARE\Better Cloud Solutions",
        "HKLM:\SOFTWARE\WOW6432Node\Better Cloud Solutions",
        "HKLM:\SOFTWARE\BetterCloudSolutions",
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*WebNavigatorBrowser*",
    ...
    RegistryHKUPatterns = @(
        ...
        "*\Software\BetterCloudSolutions",
        "*\Software\Better Cloud Solutions",
        "*\Software\Microsoft\Windows\CurrentVersion\Run*webnavigatorbrowser*",
        "*\Software\Microsoft\Windows\CurrentVersion\Internet Settings\ProxyServer",
        "*\Software\Microsoft\Windows\CurrentVersion\Internet Settings\ProxyEnable",
        "*\Software\Microsoft\SystemCertificates\Root\Certificates\" +
            "0174E68C97DDF1E0EEEA415EA336A163D2B61AFD"
    )
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Network Indicators
    # ----------------------------------------------------------------------------
    ...
    NetworkIOCs = @{
        ...
        URLs = @(
            "https://api.webbrowserbase.com/log/imp/e/chr_install_started/*",
            "https://api.webbrowserbase.com/cache/ut/"
        )
    }
    
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - File Hashes (SHA256)
    # ----------------------------------------------------------------------------
    
    # Known malicious file hashes
    FileHashes = @{
        SHA256 = @(
            "4409578c2009def377b07c0b3c28c9c941a66f5d2a592da8e817938bbd680e48",
            "044658c16c6ac0ff0ab31d8f697beda9289243a67a575fbcd84dfaadff9d13c9",
            "0f8f7fcfa958d5e13b968636aab7e35cbd3a04546b4dc2850a27837b77c6d03d",
            "e64374d205693e1f7ce8628477f46002578e67b10b3925c71f31dc76ceb8b212",
            "eeb6716b84d96e9e06ccbb5488ee38453b814a0e64cc5845fa772508871b7773",
            "6fdfcd051075383b28f5e7833fde1bb7371192f54e381c8bdfa8e68269e3dc30",
            "427d94cea702b584566425c155f3f729fe573a8e44188ad8d178d2f9695e8fbd",
            "99d55a8ee193c62f00f2d3fbde5e745842a4515de192f0b6fbd72037c31584e7"
        )
        
        MD5 = @(
            "bacf0bc2b971aa492865634bc5f79138",
            "163671853197da547981b61f790a6532",
            "b7fb52896eb928ad942cfc957e1c5930",
            "dc76f3e8a2f50970108255024882507f",
            "bab1f6feafafcc52736c51d842bfd06a"
        )
    }
    
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Browser Extensions
    # ----------------------------------------------------------------------------
    
    # Malicious browser extensions
    BrowserExtensions = @(
        "Install_Checker_1.0.0.crx",
        "production_ext_sf_WindowPop_WinOpener_1.0.0.crx",
        "*WebNavigatorBrowser*"
    )
}
