...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "Trojan.AgentB/PUA.ProfiConverter"
        FirstSeen = "2024-03-30"
        Severity = "HIGH"
        Description = "NSIS-based trojan installer disguised as PDF editor/converter software. Installs Profi Converter PUA with potential adware/unwanted software components. Multiple variants detected under names like PDFEditor, ReadSlick, SlickPDFReader, and ClearEditPro."
        CVSS = "7.8"
        ThreatIntelSource = "VirusTotal - 33/76 detections"
        SHA256 = "85a63a8254037a9260f012ee7f95c89f5eba9aca86be6b4bbb38298ef4e5b7e3"
        MD5 = "c76165815660dd1d96f74caa061ac2fb"
    }

    Name = "ProfiConverter/PDFEditor Trojan"
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "B8B7FC532DA13A614BECFE491789A495A4500B31",  # International Holdings, LLC
            "284a1c677b4200b46b269d5da883fb4639bd1252"   # myprofessionalconvert.com TLS cert
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "6B55C088776EB0B63A8146DF",           # International Holdings, LLC
            "00d609eb51943b484e0e0e93828e53eb7b"  # myprofessionalconvert.com
        )
    ...
    UserPaths = @(
        "$env:LOCALAPPDATA\ProfiConverter\Profi Converter.exe",
        "$env:LOCALAPPDATA\ProfiConverter\uninstall.exe",
        "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\ProfiConverter\Profi Converter.lnk",
        "$env:USERPROFILE\Desktop\Profi Converter.lnk"
    )
    ...
    DownloadPatterns = @(
        ...
        "*ClearEditPro*.exe",
        "*gfom1g.exe",
        "*ProfiToolInstaller*.exe"
    )
    
    # System-level paths
    SystemPaths = @(
        "$env:LOCALAPPDATA\ProfiConverter",
        "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\ProfiConverter",
        "$env:TEMP\ns*.tmp",  # NSIS temp folders
        "$env:TEMP\gettool.zip",
        "$env:TEMP\ToolFiles.zip"
    )
    ...
    RegistryHKLM = @(
        "SOFTWARE\ProfiConverter",
        "SOFTWARE\WOW6432Node\ProfiConverter",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ProfiConverter"
    )
    ...
    RegistryHKUPatterns = @(
        "Software\ProfiConverter",
        "Software\Microsoft\Windows\CurrentVersion\Uninstall\ProfiConverter",
        "Software\Classes\Applications\PDFEditor*.exe",
        "Software\Classes\Applications\ReadSlick*.exe",
        "Software\Classes\Applications\SlickPDFReader*.exe",
        "Software\Classes\Applications\ClearEditPro*.exe"
    )
    ...
    BrowserStartMenuPatterns = @(
        "*myprofessionalconvert.com*",
        "*get.myprofessionalconvert.com*",
        "*ProfiConverter*"
    )
    ...
    # Network-based IOCs (for logging/blocking)
    NetworkIOCs = @{
        Domains = @(
            "myprofessionalconvert.com",
            "get.myprofessionalconvert.com"
        ...
        URLs = @(
            "https://myprofessionalconvert.com/*",
            "https://get.myprofessionalconvert.com/2026/pro/gettool*",
            "https://get.myprofessionalconvert.com/*"
        )
    }

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - File Artifacts
    # ----------------------------------------------------------------------------

    # Specific dropped files to target
    DroppedFiles = @(
        "*\ProfiConverter\gettool.zip",
        "*\ProfiConverter\ToolFiles.zip",
        "*\ProfiConverter\d3dcompiler_47.dll",
        "*\ProfiConverter\ffmpeg.dll",
        "*\ProfiConverter\libEGL.dll",
        "*\ProfiConverter\libGLESv2.dll",
        "*\ProfiConverter\vulkan-1.dll",
        "*\ProfiConverter\vk_swiftshader.dll",
        "*\ProfiConverter\icudtl.dat",
        "*\ProfiConverter\snapshot_blob.bin",
        "*\ProfiConverter\v8_context_snapshot.bin",
        "*\ProfiConverter\resources.pak",
        "*\ProfiConverter\chrome_*.pak"
    )

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Detection Heuristics
    # ----------------------------------------------------------------------------
    ...
    DetectionHeuristics = @{
        ...
        InternalName = "ProfiToolInstaller"
        ProductName = "ProfiToolInstaller"
        FileVersion = "5.0.7.8"
    }
}
