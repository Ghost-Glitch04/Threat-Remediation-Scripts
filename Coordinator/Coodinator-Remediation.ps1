...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "ZoomInfo/Python Trojan"
        FirstSeen = "2023-08-07"
        Severity = "HIGH"
        Description = "Malicious Python-based trojan disguised as ZoomInfo application. Creates fake Google Updater services and processes. Digitally signed with expired/invalid ZoomInfo Technologies LLC certificate."
        CVSS = "7.8"
        ThreatIntelSource = "VirusTotal - 20/76 detections (coordinator.exe), 5/73 detections (ZIP archive)"
    }

    Name = "ZoomInfo Python Trojan (swapper64-2258)"
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "12726B2FDD0D8A82C092AC508C60887D01E8E2C2"  # ZoomInfo Technologies LLC (expired)
        )
    ...
    UserPaths = @(
        ...
        "*\AppData\Local\Temp\tso5h0dp.w51\*",
        "*\AppData\Local\Temp\*\2258\*"
    )
    ...
    # System-level paths
    SystemPaths = @(
        "$env:ProgramFiles\Google*_*",
        "$env:ProgramFiles\Google[0-9]*_[0-9]*",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\126.0.6462.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\127.0.6490.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\136.0.7079.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\138.0.7156.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\138.0.7194.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\140.0.7272.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\140.0.7273.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\141.0.7340.0",
        "${env:ProgramFiles(x86)}\Google\GoogleUpdater\141.0.7376.0",
        "$env:TEMP\*\2258",
        "$env:TEMP\tso5h0dp.w51"
    )
