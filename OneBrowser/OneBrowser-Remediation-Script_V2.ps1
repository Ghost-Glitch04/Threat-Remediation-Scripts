...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author             = "sentinelrshuser"
        ThreatFamily       = "OneBrowser / OneBUpdateService (PUA/Adware - Browser Hijacker)"
        FirstSeen          = "2023-10"
        Severity           = "CRITICAL"
        Description        = "OneBrowser is a Chromium-based browser hijacker that installs a persistent update service (OBUpdateService / OneBUpdateService), registers scheduled tasks, and beacons to oneinternetbrowser.com. Flagged by 10-13 AV vendors. One sample matches win_blacksuit_auto YARA rule."
        CVSS               = "8.5"
        ThreatIntelSource  = "VirusTotal - 10-13 detections across 5 submitted samples"
    }
    ...
    # ----------------------------------------------------------------------------
    # Sourced from: meaningful_name, names[], and process names observed in sandbox

    Processes = @(
        "OneBrowser",
        "OneBUpdateService",    # SHA256: 24c47dd3c0f974cf... (13 detections)
        "OBUpdateService",      # SHA256: 89ac6f31f0fc7209... (12 detections)
        "mini_installer",       # SHA256: 9b0a20c640efdc33... (10 detections) - dropper
        "setup"                 # SHA256: a89119306a39c1cb... (12 detections) - Chromium-based installer
    )

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Services
    # ----------------------------------------------------------------------------
    # setup.exe imports CreateServiceW / ChangeServiceConfigW / DeleteService
    # OBUpdateService runs as a resident service (mutex: OBResident*)
    ...
    Certificates = @{
        ...
        # NOTE: Samples carry valid Authenticode signatures - thumbprints must be
        # confirmed against observed certs before removal. Populate after cert dump.
        MaliciousThumbprints = @(
            "32A3AD63140B38D99176B3E0885903696A098D07" # OneBUpdateService.exe (observed in VT sample 24c47dd3c0f974cf1962a3871dcf940ec206da673a1bc103954086b2d8b89f30)
            "7612888362AD683E5366E63846A3A219C1DA250D" # OBUpdateService.exe (observed in VT sample 89ac6f31f0fc7209377a92eadcb71d6af88b7d21720c4ca62b7957d677d3003d)
        )

        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "0EF90B206A1B0782E0D0FDD338824EC42" # OneBUpdateService.exe (observed in VT sample 24c47dd3c0f974cf1962a3871dcf940ec206da673a1bc103954086b2d8b89f30)
            "2CA80AA74AA5EC29E23713A23D5C29A68" # OBUpdateService.exe (observed in VT sample 89ac6f31f0fc7209377a92eadcb71d6af88b7d21720c4ca62b7957d677d3003d)
        )
        ...
        SuspiciousKeywords = @(
            ...
            "oneinternetbrowser",
            "OBUpdate",
            "GoGetSSL"          # Issuing CA observed in OneBUpdateService cert chain
        )
    ...
    # ----------------------------------------------------------------------------
    # Observed directly in existing remediation script and sandbox artifacts

    TaskPatterns = @(
        "OBUpdate",             # C:\Windows\System32\Tasks\OBUpdate (confirmed in script)
        "OneBUpdate",
        "OneBrowser",
        "One Browser"
    )
    ...
    # ----------------------------------------------------------------------------
    # setup.exe imports RegSetValueExW; OBUpdateService runs resident - likely
    # sets Run key for persistence
    ...
    RegisteredAppPatterns = @(
        "OneBrowser",
        "OneBrowserHTM",            # Observed in setup.exe strings: "OneBrowserHTMOneBrowser HTML Document"
        "OneBrowserPDF"             # Observed in setup.exe strings: "OneBrowserPDFOneBrowser PDF Document"
    )

    # User-specific paths (exact matches only)
    UserPaths = @(
        "AppData\Local\OneBrowser",
        "AppData\Roaming\Microsoft\Windows\Start Menu\Programs\OneBrowser.lnk"
    )
    ...
    # System-level paths
    SystemPaths = @(
        "C:\Program Files (x86)\OneBrowser",                        # Confirmed: "Creates files inside the program directory"
        "C:\Program Files (x86)\OneBrowser\Temp",                   # Confirmed: temp dir observed in sandbox
        "C:\Windows\SystemTemp\chromium_installer.log",             # Confirmed: installer log observed in both setup.exe reports
        "C:\Windows\System32\Tasks\OBUpdate"                        # Confirmed scheduled task path
    )
    ...
    # Registry key patterns (HKLM)
    RegistryHKLM = @(
        "SOFTWARE\OneBrowser",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\OneBrowser",
        "SOFTWARE\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\TREE\OBUpdate",
        "SOFTWARE\RegisteredApplications"   # OneBrowserHTM / OneBrowserPDF file assoc entries
    )

    # Registry patterns for user hives (HKU)
    RegistryHKUPatterns = @(
        "Software\OneBrowser",
        "Software\Microsoft\Windows\CurrentVersion\Uninstall\OneBrowser",
        "Software\Microsoft\Windows\CurrentVersion\Run\OneBrowser",
        "Software\Microsoft\Windows\CurrentVersion\Run\OBUpdateService",
        "Software\Microsoft\Windows\CurrentVersion\Run\OneBUpdateService"
    )
    ...
    # ----------------------------------------------------------------------------
    # File association tracking - setup.exe registers OneBrowserHTM and OneBrowserPDF

    ApplicationAssociationPatterns = @(
        "OneBrowserHTM",        # "OneBrowserHTMOneBrowser HTML Document" - setup.exe strings
        "OneBrowserPDF",        # "OneBrowserPDFOneBrowser PDF Document"  - setup.exe strings
        "OneBrowser"
    )
    ...
    # ----------------------------------------------------------------------------
    # GUIDs observed in setup.exe strings:
    # {7D2B3E1D-D096-4594-9D8F-A6667F12E0AC}
    # {A2DF06F9-A21A-44A8-8A99-8B9C84F29160}
    # Browser version observed: 137.0.7151.69 (Chromium-based)

    FeatureUsagePatterns = @(
        "OneBrowser",
        "OBResident",           # Mutex prefix: OBResidentf669d9976489480cb6dc31be79f4de66
        "{7D2B3E1D-D096-4594-9D8F-A6667F12E0AC}",   # OneBrowser COM GUID - setup.exe
        "{A2DF06F9-A21A-44A8-8A99-8B9C84F29160}"    # OneBrowser COM GUID - setup.exe
    )
}

# ----------------------------------------------------------------------------
# SUPPLEMENTAL IOC REFERENCE (not config fields - for analyst reference)
# ----------------------------------------------------------------------------
# FILE HASHES (MD5):
#   0bfd77a15b9c472bd95762e8b330fae2  - OneBUpdateService.exe  (13 detections)
#   593be1cb75cac3cc6112e1caf304683d  - OBUpdateService.exe    (12 detections)
#   b27840942d040eeb43f060c75a7df26d  - mini_installer / onebrowser.exe (10 detections)
#   a4fb8a6698eaa343a6777141a6cc375f  - setup.exe              (12 detections)
#
# SHA256:
#   24c47dd3c0f974cf1962a3871dcf940ec206da673a1bc103954086b2d8b89f30  - OneBUpdateService.exe
#   89ac6f31f0fc7209377a92eadcb71d6af88b7d21720c4ca62b7957d677d3003d  - OBUpdateService.exe
#   9b0a20c640efdc33a542c58a1ccc9120e6fb3dda256095505d03557e347f4f04  - mini_installer
#   a89119306a39c1cba6055548507ad3140d04288b4a2b19087d242e68a94548b0  - setup.exe
#
# C2 / NETWORK:
#   oneinternetbrowser.com               - Primary C2 / update server
#   https://oneinternetbrowser.com/service/update2   - Update beacon URL
#   162.159.36.2                         - IP contacted without DNS (Cloudflare-routed)
#
# MUTEX:
#   \Sessions\1\BaseNamedObjects\OBResidentf669d9976489480cb6dc31be79f4de66
#   \Sessions\1\BaseNamedObjects\One Browser Installer
#
# YARA MATCH (OBUpdateService.exe only):
#   win_blacksuit_auto (Malpedia) - ruleset 0084743690
#
# CONFIG FILE DROPPED:
#   *_OneBUpdate.conf  (e.g. 6a3e0fc6_OneBUpdate.conf, 739e1002_OBUpdate.conf)
