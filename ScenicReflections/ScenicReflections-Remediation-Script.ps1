...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "ScenicReflections / Revoked Certificate Malware"
        FirstSeen = "2009-12-05"
        Severity = "HIGH"
        Description = "NSIS installer signed with revoked certificate from 'Bit Wise Publishing, LLC'. Detected as potentially malicious (8/76 detections). Uses invalid code signing certificates that have been revoked and are not time valid."
        CVSS = "7.5"
        ThreatIntelSource = "VirusTotal - 8/76 detections"
        FileHashes = @{
            MD5 = "ff0b4f921c89f961e83319d85881308c"
            SHA1 = "dad68d131aec9d67d65eff78b0840777eea5be0e"
            SHA256 = "6945269e20749016f7a78660b70657b2098945a40040082b04a27bda029d2354"
        }
    }

    Name = "ScenicReflections"
    ...
    Processes = @(
        "ScenicReflections",
        "7BDC821B18ADC4FA6E5306EF10ECCF0026B321A1"
    )
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "BDA8276DB0C2D007FA2724D67517FE5BB71BAAB1"  # Bit Wise Publishing, LLC
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            # Bit Wise Publishing, LLC - with spaces
            "44 DC E4 7A 76 86 99 E5 60 7D 54 CB 97 12 05 00",
            # Bit Wise Publishing, LLC - without spaces
            "44DCE47A768699E5607D54CB97120500",
            
            # COMODO Time Stamping Signer - with spaces
            "47 8A 8E FB 59 E1 D8 3F 0C E1 42 D2 A2 87 07 BE",
            # COMODO Time Stamping Signer - without spaces
            "478A8EFB59E1D83F0CE142D2A28707BE",
            
            # Sectigo (UTN Object) - with spaces
            "44 BE 0C 8B 50 00 24 B4 11 D3 36 2D E0 B3 5F 1B",
            # Sectigo (UTN Object) - without spaces
    ...
    UserPaths = @(
        "ScenicReflections.exe",
        "7BDC821B18ADC4FA6E5306EF10ECCF0026B321A1.exe"
    )
    ...
    # Download folder patterns (specific to ScenicReflections)
    DownloadPatterns = @(
        "*ScenicReflections*",
        "*7BDC821B18ADC4FA6E5306EF10ECCF0026B321A1*",
        "*ff0b4f921c89f961e83319d85881308c*"
    )
    
    # System-level paths
    SystemPaths = @(
        "$env:ProgramFiles\ScenicReflections",
        "$env:ProgramFiles(x86)\ScenicReflections",
        "$env:ProgramFiles\Bit Wise Publishing",
        "$env:ProgramFiles(x86)\Bit Wise Publishing",
        "$env:ProgramData\ScenicReflections",
        "$env:ProgramData\Bit Wise Publishing",
        "$env:CommonProgramFiles\ScenicReflections",
        "$env:CommonProgramFiles(x86)\ScenicReflections"
    )
    ...
    RegistryHKLM = @(
        "SOFTWARE\ScenicReflections",
        "SOFTWARE\Bit Wise Publishing",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ScenicReflections",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Bit Wise Publishing",
        "SOFTWARE\WOW6432Node\ScenicReflections",
        "SOFTWARE\WOW6432Node\Bit Wise Publishing",
        "SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\ScenicReflections",
        "SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Bit Wise Publishing"
    )
    ...
    RegistryHKUPatterns = @(
        "Software\ScenicReflections",
        "Software\Bit Wise Publishing",
        "Software\Microsoft\Windows\CurrentVersion\Run\ScenicReflections",
        "Software\Microsoft\Windows\CurrentVersion\Run\*ScenicReflections*"
    )
