...
# ----------------------------------------------------------------------------
# MALWARE CONFIGURATION - Weaponized Datto RMM / CentraStage Agent
# ----------------------------------------------------------------------------
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author             = "sentinelrshuser"
        ThreatFamily       = "hacktool.datto"
        FirstSeen          = "2016-04-26"
        Severity           = "CRITICAL"
        Description        = @"
Weaponized Datto RMM (CentraStage) agent installer distributed via social engineering.
Legitimately signed by 'Datto, LLC' via DigiCert code-signing chain, this NSIS-packaged
Win32 EXE installs a full remote monitoring and management (RMM) agent suite including
VNC remote control (SmartCode ViewerX), AEMAgent, firewall rule manipulation, and
persistent service installation. Distributed under lure names such as
'RSVP_Invite_access_2026.exe'. The agent registers with attacker-controlled CentraStage
infrastructure (vidal-agent.centrastage.net), establishes persistence via Windows service
(CagService), Run key, Safe Boot survival, and COM object registration. Drops ~100+ files
including PuTTY, VNC controls, and WebRemote components. Modifies firewall rules via
netsh to allow C2 communications. Tagged: persistence, signed, executes-dropped-file,
overlay, nsis, peexe.
"@
        CVSS               = "8.5"
        ThreatIntelSource  = "VirusTotal - 10/76 detections"
    }

    Name = "hacktool.datto (Weaponized Datto RMM / CentraStage)"

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Hashes (for validation/hunting)
    # ----------------------------------------------------------------------------

    Hashes = @{
        SHA256 = "902ab3ee6746647911371e56ecd3d90dcfe3d5f79e8f6c4b1ad4384f4b26bf5f"
        SHA1   = "2aa9ac0a5fe814be5dd192c2afaaa51df1d00145"
        KnownFileNames = @(
            "902ab3ee6746647911371e56ecd3d90dcfe3d5f79e8f6c4b1ad4384f4b26bf5f.exe",
            "RSVP_Invite_access_2026.exe",
            "RSVP_Invite_access_2026 (2).exe",
            "5rcdhz9.exe",
            "wshvg8xp0.exe",
            "nddg42kk.exe",
            "k8eoucl.exe"
    ...
    Certificates = @{
        # Known malicious certificate thumbprints — the Datto code-signing leaf cert
        # used to sign this weaponized installer (PRIORITY 1 - REMOVE)
        MaliciousThumbprints = @(
            "BECD318DCDD7363CC2E9782B52C33A62FE1BFD90"   # Datto, LLC code-signing cert
        )

        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "01 2A 9B 58 57 13 97 95 6C DC FC 20 20 3A CF 49"  # Datto, LLC code-signing
        )
    ...
    # Download/lure file patterns
    DownloadPatterns = @(
        "*RSVP_Invite_access*",
        "*902ab3ee6746647911371e56ecd3d90dcfe3d5f79e8f6c4b1ad4384f4b26bf5f*",
        "*5rcdhz9*",
        "*wshvg8xp0*",
        "*nddg42kk*",
        "*k8eoucl*"
    ...
    # System-level paths to remove
    SystemPaths = @(
        "${env:ProgramFiles(x86)}\CentraStage",
        "$env:ProgramData\CentraStage",
        "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\CentraStage",
        "$env:SystemRoot\System32\config\systemprofile\AppData\Local\CentraStage"
    )

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Network IOCs (for firewall/DNS blocking)
    # ----------------------------------------------------------------------------
    ...
    NetworkIOCs = @{
        ...
        URLs = @(
            "https://vidal-agent.centrastage.net/cs/services/CentraStage",
            "https://update-vidal.centrastage.net/putty/version",
            "https://update-vidal.centrastage.net/cagupdate/aem-agent/version.json",
            "https://update-vidal.centrastage.net/cagupdate/aem-agent/10338/win/AEMAgentDeploy.netcore-win-x64.14.8.0.10338.zip",
            "https://update-vidal.centrastage.net/cagupdate/aem-agent/10349/win/AEMAgentDeploy.netcore-win-x64.14.8.1.10349.zip",
            "https://update-vidal.centrastage.net/web-rto/version.json",
            "https://update-vidal.centrastage.net/web-rto/RMM.WebRemote.netcore-win-x64.14.8.0.3881.zip",
            "https://update-vidal.centrastage.net/web-rto/RMM.WebRemote.deps.netcore-win-x64.14.8.0.3881.zip",
            "https://features.vidal.rmm.datto.com/proxy",
            "https://agent-gateway.vidal.rmm.datto.com/audit/key/account/vida8920001/device/31b5660c-be82-5092-a209-74d8bcc6bda7",
            "https://vidal-agent-notifications.centrastage.net/device/31b5660c-be82-5092-a209-74d8bcc6bda7/notification",
            "https://vidal-agent-comms.centrastage.net/200"
        )
        DeviceID = "31b5660c-be82-5092-a209-74d8bcc6bda7"
        AccountID = "vida8920001"
    }

    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Firewall Rules to Remove
    # ----------------------------------------------------------------------------

    FirewallRulePatterns = @(
        "CentraStage_monitor",
        "CentraStage_service",
        "AEMAgent",
        "RMM.WebRemote",
        "RMM.WebRemote 14.8.0.3881",
        "Aria"
    )
    ...
    RegistryHKLM = @(
        ...
        "HKLM:\SOFTWARE\Classes\cag",
        "HKLM:\SOFTWARE\Classes\AppID\scvncctrl.dll",
        "HKLM:\SOFTWARE\Classes\AppID\{2864468C-765F-4EF3-B8CF-42666C92B545}",

        # SmartCode VNC viewer COM registrations
        "HKLM:\SOFTWARE\Classes\SmartCode.ViewerX",
        "HKLM:\SOFTWARE\Classes\SmartCode.ViewerX.1",
        "HKLM:\SOFTWARE\Classes\SmartCode.ViewerX.2",
        "HKLM:\SOFTWARE\Classes\SmartCode.ViewerX.3",

        # WOW6432Node CLSID for VNC control
        "HKLM:\SOFTWARE\Classes\WOW6432Node\CLSID\{62FA83F7-20EC-4D62-AC86-BAB705EE1CCD}",
        "HKLM:\SOFTWARE\Classes\Wow6432Node\CLSID\{62FA83F7-20EC-4D62-AC86-BAB705EE1CCD}",

        # TypeLib registration
        "HKLM:\SOFTWARE\Classes\TypeLib\{2864468C-765F-4EF3-B8CF-42666C92B545}",
        "HKLM:\Software\Classes\TypeLib\{2864468C-765F-4EF3-B8CF-42666C92B545}",

        # WOW6432Node Interface registrations (VNC control interfaces)
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{200722C4-0B36-48F7-BACF-A9E7AD3D0A7E}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{301C38CE-584C-4E08-A545-140A08892FB0}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{37A0E3B1-1AFD-4125-AB2E-2DE8F69DED39}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{5D2B76F9-B1F4-43B5-A207-F23EA608BA49}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{82B3A16E-4E01-467E-AED4-198EA092D3AF}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{C35468F3-A938-4CB4-ADBB-D0BB37F504CA}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{CC783088-B1D2-4B2F-88CA-51B8ACC638B9}",
        "HKLM:\SOFTWARE\Classes\WOW6432Node\Interface\{FE105750-449E-4220-ADF5-AB8EF7D6906A}",
        "HKLM:\SOFTWARE\Classes\Wow6432Node\Interface\{200722C4-0B36-48F7-BACF-A9E7AD3D0A7E}",
        "HKLM:\SOFTWARE\Classes\Wow6432Node\Interface\{301C38CE-584C-4E08-A545-140A08892FB0}",
        "HKLM:\SOFTWARE\Classes\Wow6432Node\Interface\{37A0E3B1-1AFD-4125-AB2E-2DE8F69DED39}",
        "HKLM:\SOFTWARE\Classes\Wow6432Node\Interface\{5D2B76F9-B1F4-43B5-A207-F23EA608BA49}",
        ...
        # Native Interface registrations (mirrored)
        "HKLM:\Software\Classes\Interface\{200722C4-0B36-48F7-BACF-A9E7AD3D0A7E}",
        "HKLM:\Software\Classes\Interface\{301C38CE-584C-4E08-A545-140A08892FB0}",
        "HKLM:\Software\Classes\Interface\{37A0E3B1-1AFD-4125-AB2E-2DE8F69DED39}",
        "HKLM:\Software\Classes\Interface\{5D2B76F9-B1F4-43B5-A207-F23EA608BA49}",
        "HKLM:\Software\Classes\Interface\{82B3A16E-4E01-467E-AED4-198EA092D3AF}",
        "HKLM:\Software\Classes\Interface\{C35468F3-A938-4CB4-ADBB-D0BB37F504CA}",
        "HKLM:\Software\Classes\Interface\{CC783088-B1D2-4B2F-88CA-51B8ACC638B9}",
        "HKLM:\Software\Classes\Interface\{FE105750-449E-4220-ADF5-AB8EF7D6906A}",
        "HKLM:\SOFTWARE\Classes\Interface\{200722C4-0B36-48F7-BACF-A9E7AD3D0A7E}",
        "HKLM:\SOFTWARE\Classes\Interface\{301C38CE-584C-4E08-A545-140A08892FB0}",
        "HKLM:\SOFTWARE\Classes\Interface\{37A0E3B1-1AFD-4125-AB2E-2DE8F69DED39}",
        "HKLM:\SOFTWARE\Classes\Interface\{5D2B76F9-B1F4-43B5-A207-F23EA608BA49}",
        ...
        # Uninstall entry
        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\CentraStage",
        "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\CentraStage",
        "HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\CentraStage",
        "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\CentraStage",

        # App Paths registration
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\CagService.exe",
        "HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\App Paths\CagService.exe",
        "HKLM:\Software\Microsoft\Windows\CurrentVersion\App Paths\CagService.exe",

        # Service registration
        "HKLM:\SYSTEM\CurrentControlSet\Services\CagService",
        "HKLM:\SYSTEM\ControlSet001\Services\CagService",
        "HKLM:\SYSTEM\ControlSet001\services\CagService",

        # Safe Boot survival key
        "HKLM:\SYSTEM\CurrentControlSet\Control\SafeBoot\Network\CagService",
        "HKLM:\SYSTEM\ControlSet001\Control\SafeBoot\Network\CagService",

        # Event log source
        "HKLM:\SYSTEM\CurrentControlSet\Services\EventLog\Application\CagService",
        "HKLM:\SYSTEM\ControlSet001\Services\EventLog\Application\CagService",

        # Tracing keys
        "HKLM:\SOFTWARE\Microsoft\Tracing\CagService_RASAPI32",
        "HKLM:\SOFTWARE\Microsoft\Tracing\CagService_RASMANCS"

        # Policies modified (SoftwareSASGeneration = 3 allows software-generated SAS)
        # NOTE: This value should be RESET to 0 or removed, not the entire key
        # "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" # -- value SoftwareSASGeneration
    )

    # Registry values to specifically remove (key + value name pairs)
    RegistryHKLMValues = @(
        @{
        ...
        @{
            ...
            Note  = "Reset to 0 - malware sets this to 3 to allow software SAS generation"
        }
    )
    ...
    RegistryHKUPatterns = @(
        ...
        "*\AppData\Local\CentraStage\*"
    )

    # Specific HKU keys observed in sandbox
    RegistryHKU = @(
        # Per-user cag:// protocol handler
        "HKU:\*_Classes\cag",
        "HKU:\*_Classes\cag\URL Protocol",
        "HKU:\*_Classes\cag\shell",
        "HKU:\*_Classes\cag\shell\open",
        "HKU:\*_Classes\cag\shell\open\command"
    )
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Dropped File Hashes (for hunting/validation)
    # ----------------------------------------------------------------------------

    DroppedFileHashes = @{
        "CagService.exe"   = "eb209f6eccdebca405183de5f24ef34ee8c850615e6ab9f086f5a4fb6851e071"
        "Gui.exe"          = "5cb69d880a0ceb97926036debb3e01ea2095e8766f49cedf27658a18002cf217"
        "scvncctrl.dll"    = "e9932fb947df5125648828248f51beb1f0b213b41f0a15bf77d56bc4b9217375"
        "nsProcess.dll"    = "95fe9d92512ff2318cc2520311ef9145b2cee01209ab0e1b6e45c7ce1d4d0e89"
        "putty.exe"        = "e61b8f44ab92cf0f9cb1101347967d31e1839979142a4114a7dd02aa237ba021"
        "CsExec.Service.exe" = "88773a254a7e056a83d6eb7d80b57cc0195cc16bca8317ea35a0576b6a38c639"
        "uninst.exe"       = "dc6cf05cde0f011869a289faa32a791994e200a3fbaff3e30678d67672ed3543"
    }
}
