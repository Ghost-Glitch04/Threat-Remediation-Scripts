# ============================================================================
# PDFMaestro - MalwareConfig
# Version    : 1.0.0
# Date       : 2026-07-15
# Author     : Ghost
# Sources    : VT_Report_20260715_165114.txt (VirusTotal sandbox report)
#              threats_2523362465382443557.csv (SentinelOne EDR export)
# SHA256     : 4eedbd0f6511d3984abecc3b4a0da43e53d18190cc3cb55eef2d40a30bf3b1e8
# SHA1       : 31954ae135d8cdc6d76234a0ba019b0a17a8d4a3
# MD5        : a11c7e726f87bef514fa2bcb3fa37022
# ============================================================================
#
# DEPLOYMENT CAVEATS — READ BEFORE EXECUTING
# ------------------------------------------
# CAV-001  Network IOC (172.64.80.1) is a Cloudflare BGP range also used by
#          legitimate CDN traffic. Do NOT use as a kill criterion. Recorded
#          for hunt/correlation only. SRC: SentinelOne EDR > Network Actions.
#
# CAV-002  Certificate chain above the leaf (DigiCert Trusted G4 Code Signing
#          RSA4096 SHA384 2021 CA1, DigiCert Trusted Root G4, DigiCert) are
#          shared infrastructure. Removal MUST target ONLY the leaf thumbprint
#          E69B09DD553C6094907B46DE2925751E6320BED5 (Secure PC Software LLC).
#          Do NOT remove intermediate or root certs. SRC: VT > signature_info.
#
# CAV-003  Compilation timestamp in PE header is spoofed to 2096-10-07 —
#          a known timestomping technique. Do not rely on file date for triage.
#          SRC: VT > CAPE Sandbox > pe_compile_timestomping (MEDIUM).
#
# CAV-004  Binary contains anti-analysis strings referencing sandbox/debugger
#          tool names (IDA, x64dbg, Wireshark, JoeBox, etc.). Sandbox results
#          may be incomplete if evasion triggered. SRC: VT > CAPA > id:4.
#
# CAV-005  PDFMaestro.db-shm and .db-wal files observed in EDR are SQLite
#          write-ahead log components. Rapid churn (create/delete cycles at
#          1-second intervals) is consistent with an application polling loop,
#          NOT ransomware encryption. No path-pattern block on these alone;
#          scope removal to the parent directory \AppData\Local\SB\PM\.
#          SRC: SentinelOne EDR > Files.
#
# CAV-006  Behavioral indicator "Remote Memory Free" flagged against lsass.exe
#          (PID 1656). This warrants post-remediation credential audit on
#          affected host (jls667 / blsteiner). Not reflected in file cleanup.
#          SRC: SentinelOne EDR > Indicators.
#
# CAV-007  SentinelOne Threat ID 2523362465382443557 was mitigated by Agent
#          Policy on 2026-07-14 13:50:48 UTC. Verify mitigation status before
#          re-running destructive phases on that host.
#          SRC: SentinelOne EDR > Threat Information.
#
# CAV-008  Sections below left empty (Services, TaskPatterns, RunKeyPatterns,
#          RegisteredAppPatterns, RegistryHKLM, RegistryHKUPatterns,
#          BrowserStartMenuPatterns, ApplicationAssociationPatterns,
#          FeatureUsagePatterns, DownloadPatterns) have no positive evidence
#          in either source. Do NOT populate from assumption. Requires
#          live-host field telemetry or deeper sandbox run to close.
#
# ============================================================================
