...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "trojan.evilai / NSIS Installer Trojan"
        FirstSeen = "2026-01-24"
        Severity = "CRITICAL"
        Description = "NSIS-based trojan installer disguised as 'Food Formula' application. Downloads payloads from foodformulasetup.com, installs to AppData\Roaming\FoodFormula, and exhibits clipboard monitoring, VM detection, and data exfiltration capabilities."
        CVSS = "8.5"
        ThreatIntelSource = "VirusTotal - 2/75 detections"
        SHA256 = "b179bec73efe194bffb69eeedc8c136b5e7506a10b3589d223b1e6f8a8fbfb53"
        MD5 = "e0817b5012726efcc854bb396259823c"
    }

    Name = "FoodFormula Trojan"
    ...
    Processes = @(
        "FoodFormula_*",           # Main installer (randomized numeric suffix)
        "Food Formula",            # Installed executable
        "KitchenCanvas_*",         # Variant installer name
        "uninstall"                # Associated uninstaller in FoodFormula folder
    )
    ...
    Services = @(
        # No persistent services detected in VT analysis
    )
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "52E8859904BAC80770A2B4D13DDC9CAE32FA7EEE",  # INSTALLERIM LLC code signing cert
            "3F36521B88954ABC8BB35E838D467E881D8358D1"   # foodformulasetup.com TLS cert
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "40398C6509D383F3D517DA8A",                  # INSTALLERIM LLC
            "6EE14A0B4D10670713888DD7FCA33F53"           # foodformulasetup.com (WE1)
        )
        ...
        ProtectedKeywords = @(
            ...
            "GeoTrust",
            "Comodo",
            "GlobalSign Root",  # Note: Legitimate CA root, but issued malicious cert
            "Cloudflare"
        )
    ...
    UserPaths = @(
        "\AppData\Roaming\FoodFormula\Food Formula.exe",
        "\AppData\Roaming\FoodFormula\uninstall.exe",
        "\Desktop\Food Formula.lnk"
    )
    ...
    SystemPaths = @(
        "$env:APPDATA\FoodFormula",
        "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\FoodFormula",
        "$env:LOCALAPPDATA\Temp\nsl*.tmp",    # NSIS installer temp folders
        "$env:LOCALAPPDATA\Temp\nso*.tmp",
        "$env:LOCALAPPDATA\Temp\nsy*.tmp",
        "$env:LOCALAPPDATA\Temp\nsz*.tmp",
        "$env:LOCALAPPDATA\Temp\nsq*.tmp",
        "$env:LOCALAPPDATA\Temp\nsf*.tmp",
        "$env:USERPROFILE\Desktop\Food Formula.lnk"
    )
    ...
    RegistryHKLM = @(
        "SOFTWARE\Classes\FoodFormula",
        "SOFTWARE\Classes\KitchenCanvas",
        "SOFTWARE\WOW6432Node\FoodFormula",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\FoodFormula"
    )
    
    RegistryHKUPatterns = @(
        "Software\Microsoft\Windows\CurrentVersion\Uninstall\FoodFormula",
        "Software\Microsoft\Windows\Shell\Bags\*\Desktop\IconLayouts",  # Desktop icon pollution
        "Software\Microsoft\Windows\CurrentVersion\UFH\SHC\*",          # User File History entries
        "Software\Classes\FoodFormula"
    )
    ...
    NetworkIOCs = @{
        ...
        URLs = @(
            "https://foodformulasetup.com/*",
            "https://formulas.foodformulasetup.com/nextgen/*"
        )
        
        JA3Hashes = @(
            "446fb8bd613d17e5d44bd3b288ac94ca",
            "74954a0c86284d0d6e1c4efefe92b521",
            "2d0c1aa81856c537394b05616d38dbae",
            "2982919ca7b9911dea9344128ed99ad6"
        )
    }
    
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - MITRE ATT&CK Mapping
    # ----------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Behavioral Signatures
    # ----------------------------------------------------------------------------
    
    BehavioralSignatures = @(
        "NSIS Installer with embedded DLLs (math.dll, NScurl.dll, nsDialogs.dll)",
        "Downloads ZIP payload from remote server",
        "Unpacks Electron-based application framework",
        "Monitors clipboard data",
        "Checks for virtual machine environment (Xen detection)",
        "Excessive file retransmissions during C2 communication",
        "Creates desktop shortcuts without user consent",
        "Reads .ini configuration files",
        "Modifies Windows Shell icon layouts",
        "Queries cryptographic machine GUID",
        "Uses XOR encoding for data obfuscation",
        "Captures webcam images (capability detected)",
        "Performs mouse movement detection for sandbox evasion",
        "Establishes HTTPS connections to suspicious domains"
    )
}
