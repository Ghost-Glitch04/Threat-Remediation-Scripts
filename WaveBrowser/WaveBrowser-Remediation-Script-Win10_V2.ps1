...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author       = "sentinelrshuser"
        ThreatFamily = "WaveBrowser"  # SRC: user directive (2026-08-06) - product/binary is "Wavesor SWUpdater" (signature_info.product), installed payload is the WaveBrowser hijacker; family name set to the hijacker per Ghost's decision
        FirstSeen    = "2026-07"  # SRC: VT first_submission_date=1782887348 (2026-07-01 02:29:08 EDT). CORRECTED from template's stale "2023-10" value, which does not match any field in this VT record or PE compile timestamp (creation_date=1782844954 / 2026-06-30 14:42:34 EDT) - flagging as a possible template carryover from a prior/unrelated threat entry
        Severity     = "CRITICAL"  # SRC: VT popular_threat_classification.popular_threat_category = trojan(5)/adware(1), suggested_threat_label="trojan.wavebrowser/applicunwnt"; sigma_analysis rule_level=medium (ADS-hiding executable, rule_id 5be9da0a...); S1 threat card Classification="Malware". Template's pre-existing "CRITICAL" is retained as consistent with this evidence, not independently re-derived
        Description  = "Wavesor SWUpdater installer (SWUpdaterSetup.exe) drops and stages the WaveBrowser Chromium-based browser hijacker plus its update-service components (SWUpdaterCore, SWUpdaterBroker, SWUpdaterOnDemand, SWUpdaterComRegisterShell64, psmachine/psuser helper DLLs). Installer is Authenticode-signed by 'Wavesor Software (Eightpoint Technologies Ltd. SEZC)' but that leaf certificate is now outside its validity window. Establishes persistence via a Run key, a scheduled task, and per-user registry/COM/RegisteredApplications entries; hijacks default-browser association via StartMenuInternet client registration."  # SRC: VT signature_info, pe_info dropped-file list (files_dropped, CAPE Sandbox behavior), popular_threat_classification; cross-referenced against both .ps1 remediation scripts
        CVSS         = ""  # OPEN GAP: no CVSS scoring source was provided (VT does not supply CVSS; no NVD/vendor advisory in source set). Template's pre-existing "8.5" removed rather than carried forward unsourced, per no-fabrication rule - populate only when a scored source is supplied
        ThreatIntelSource = "VirusTotal - 21/75 detections"  # SRC: VT last_analysis_stats (malicious=21, undetected=50, type-unsupported=4; total scanners=75)
    }
 
    Name = "WaveBrowser"  # SRC: user directive (2026-08-06); internal binary name per VT signature_info.internal_name = "Wavesor SWUpdater Setup" - Name field set to the family name for consistency with Metadata.ThreatFamily, not the binary's internal name
    ...
    Processes = @(
        "WaveBrowser",  # SRC: WaveBrowser-Remediation-Script-Win10.ps1, Get-Process wavebrowser
        "SWUpdater",
        "SWUpdaterCore",
        "SWUpdaterBroker",
        "SWUpdaterOnDemand",
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "2EA4ADE8719DE01274C5A3BAF694B91E339BDA79"  # SRC: VT signature_info.signers_details[1] - leaf cert "Wavesor Software (Eightpoint Technologies Ltd. SEZC)", status: "This certificate or one of the certificates in the certificate chain is not time valid" (valid 2023-09-14 to 2026-07-20, now expired). LEAF ONLY - per cert-targeting principle, the DigiCert intermediate (7B0F360B775F76C94A12CA48445AA2D2A875701C) and root chain are NOT included; they are shared legitimate CA infrastructure and are covered by the "DigiCert" entry in ProtectedKeywords below.
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "09 D7 7A 45 C1 C0 97 55 AE 3E 7A 51 53 98 3C 03"  # SRC: VT signature_info.signers_details[1], leaf cert serial number (same cert as thumbprint above)
        )
 
        # Suspicious keywords in Subject/Issuer (PRIORITY 3 - ANALYZE)
        SuspiciousKeywords = @(
            "Wavesor",                  # SRC: VT signature_info.signers = "Wavesor Software (Eightpoint Technologies Ltd. SEZC)"
            "Eightpoint Technologies",  # SRC: same field - offshore/shell-style signer entity name pattern
            "SEZC"                      # SRC: same field - Cayman Islands "Special Economic Zone Company" suffix, common in PUA/adware signer entities; flagged for analyst review, not auto-removal
        )
    ...
    TaskPatterns = @(
        "*Wave*"  # SRC: WaveBrowser-Remediation-Script-Win10.ps1, Get-ScheduledTask -TaskName *Wave*
        # NOTE: the on-disk task folder ("C:\windows\system32\tasks\Wavesor*") and the HKLM task
        # cache registry tree are DIFFERENT artifact types from a task-name wildcard - relocated to
        # SystemPaths and RegistryHKLM respectively (see below) rather than duplicated here.
    )
    ...
    RunKeyPatterns = @(
        "Wavesor SWUpdater"  # SRC: WaveBrowser-Remediation-Script-Win10.ps1 - value name under HKU\<SID>\Software\Microsoft\Windows\CurrentVersion\Run
    )
    
    # Registered applications patterns
    RegisteredAppPatterns = @(
        "WaveBrowser.5QMLTPZDDJG2BQZHV26QUN4ZK4"  # SRC: WaveBrowser-Remediation-Script-Win10.ps1 - value name under HKU\<SID>\Software\RegisteredApplications
    )

    
    # User-specific paths (exact matches only)
    UserPaths = @(
        "Wavesor Software",                    # SRC: script, C:\users\$username\Wavesor Software
        "WebNavigatorBrowser",                 # SRC: script, C:\users\$username\WebNavigatorBrowser
        "AppData\Local\WaveBrowser",           # SRC: script, C:\users\$username\appdata\local\WaveBrowser
        "AppData\Local\WebNavigatorBrowser"    # SRC: script, C:\users\$username\appdata\local\WebNavigatorBrowser
    )
    ...
    DownloadPatterns = @(
        "Wave Browser*.exe",  # SRC: script, C:\users\$username\downloads\Wave Browser*.exe
        "Wave Browser*.msi",
        "WaveBrowser*.exe",
        "WaveBrowser*.msi",
        "SWUpdaterSetup*.exe",
    ...
    # System-level paths
    SystemPaths = @(
        "C:\Windows\System32\Tasks\Wavesor*"  # SRC: WaveBrowser-Remediation-Script-Win10.ps1, $taskPaths - on-disk scheduled task definition file(s)
        # OPEN GAP: VT CAPE/Jujubox/Zenbox sandbox runs show install staging under
        # "C:\Program Files (x86)\Wavesor\Temp\<GUID>.tmp\..." but that staging directory is
        # deleted by the installer itself post-install (files_deleted lists the .tmp folder in
        # every sandbox run) and no run confirms a final persistent path under
        # "C:\Program Files (x86)\Wavesor\". Neither remediation script targets Program Files.
        # Requires live-host audit to confirm final install location before adding a
        # System Path removal target - not populated speculatively.
    )
    ...
    RegistryHKLM = @(
        "SOFTWARE\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\TREE\Wave*"  # SRC: WaveBrowser-Remediation-Script-Win10.ps1, $taskPaths
    )

    
    # Registry patterns for user hives (HKU) - for cleanup
    RegistryHKUPatterns = @(
        "WaveBrowser",                                                              # SRC: script, HKU\<SID>\Software\WaveBrowser
        "Wavesor",                                                                  # SRC: script, HKU\<SID>\Software\Wavesor
        "WebNavigatorBrowser",                                                      # SRC: script, HKU\<SID>\Software\WebNavigatorBrowser
        "Microsoft\Windows\CurrentVersion\Uninstall\WaveBrowser",                   # SRC: script, HKU\<SID>\Software\...
        "Microsoft\Windows\CurrentVersion\App Paths\wavebrowser.exe",               # SRC: script, HKU\<SID>\Software\...
        "Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\WaveBrowser",       # SRC: script, HKU\<SID>\Software\...
        "Classes\CLSID\{9CD78CBC-FD21-4FFF-B452-9D792A58B7C4}\LocalServer32",       # SRC: script, HKU\<SID>\Software\Classes\CLSID\...
        "Clients\StartMenuInternet\WaveBrowser.5QMLTPZDDJG2BQZHV26QUN4ZK4",         # SRC: script, HKU\<SID>\Software\Clients\StartMenuInternet\... (default-browser registration)
        # --- entries below are relative to HKU\<SID>\_Classes\ (Classes root redirection hive, NOT HKU\<SID>\Software\Classes\) ---
        "_CLASSESROOT:WaveBrwsHTM*",                                                # SRC: script, HKU\<SID>_Classes\WaveBrwsHTM*
        "_CLASSESROOT:WavesorSWUpdater.CredentialDialogUser",                       # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.CredentialDialogUser.1.0",                   # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.OnDemandCOMClassUser",                       # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.OnDemandCOMClassUser.1.0",                   # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.PolicyStatusUser",                           # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.PolicyStatusUser.1.0",                       # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.Update3COMClassUser",                        # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.Update3COMClassUser.1.0",                    # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.Update3WebUser",                             # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:WavesorSWUpdater.Update3WebUser.1.0",                         # SRC: script, HKU\<SID>_Classes\...
        "_CLASSESROOT:CLSID\{9CD78CBC-FD21-4FFF-B452-9D792A58B7C4}\LocalServer32"   # SRC: script, HKU\<SID>_Classes\CLSID\... (same CLSID as the Software\Classes entry above, different hive path - both present in source script, both retained)
    )
    ...
    BrowserStartMenuPatterns = @(
        "AppData\Roaming\Microsoft\Windows\Start Menu\Programs\WaveBrowser.lnk",                          # SRC: script
        "AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\WaveBrowser.lnk",                        # SRC: script
        "Desktop\WaveBrowser.lnk",                                                                          # SRC: script
        "AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\WaveBrowser.lnk"     # SRC: script
    )
