# ============================================================================ #
# Malware Remediation Framework - PulseBrowser
# ============================================================================ #
...
$MalwareConfig = @{
    ...
    Processes = @(
        "pulsebrowser"  # SRC: VT CAPE command_executions - "...\PulseBrowser\Application\pulsebrowser.exe" --from-installer
    )
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "CB7DD993165246E52B2CCB50DC8D4779711F68E7"  # SRC: VT signature_info.signers_details - leaf cert "Alabama Technology USA, LLC", issued by Sectigo Public Code Signing CA EV R36, valid 2026-04-17 to 2027-04-17. CORROBORATED: JOE IOC list independently names "Alabama Technology USA, LLC" as Authenticode signer on the separate v133.0.6943.205 build.
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "72 D2 A0 34 9A E0 C0 00 25 6B 7C 63 40 28 8C 9B"  # SRC: VT signature_info.signers_details - serial number for the Alabama Technology USA, LLC leaf cert above
        )
    ...
    RunKeyPatterns = @(
        "PulseBrowserUpdaterTaskUser*"  # SRC: VT CAPE registry_keys_set - HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run\PulseBrowserUpdaterTaskUser135.0.7049.101 = "...\updater.exe" --wake. Also matched by VT Sigma rule "CurrentVersion Autorun Keys Modification" (medium). Wildcarded on version per TaskPatterns rationale above.
    )
    
    # Registered applications patterns
    RegisteredAppPatterns = @(
        "PulseBrowser*"  # SRC: VT CAPE registry_keys_set - HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\PulseBrowser135.0.7049.101 = "...\PulseBrowser\Application\pulsebrowser.exe" --from-installer. Also matched by VT Sigma rule "CurrentVersion Uninstall Key Modification" (medium). Wildcarded on version per TaskPatterns rationale above.
    )
    
    # User-specific paths (exact matches only)
    UserPaths = @(
        "%LOCALAPPDATA%\PulseSoftware"  # SRC: VT CAPE files_written/files_opened - parent of PulseBrowserUpdater\<version>\, PulseBrowser\Application\<version>\, and Update\ subtrees. NOTE: exact-match field cannot accommodate the embedded version-number subfolders (e.g. \135.0.7049.101\) - using the stable parent directory. If the remediation script requires deeper exact paths, the two confirmed version-specific subpaths observed were: %LOCALAPPDATA%\PulseSoftware\PulseBrowserUpdater\135.0.7049.101 and %LOCALAPPDATA%\PulseSoftware\PulseBrowser\Application\149.0.7827.158
    )
    ...
    RegistryHKUPatterns = @(
        "SOFTWARE\PulseSoftware\Update"  # SRC: VT CAPE registry_keys_set - HKCU\SOFTWARE\PulseSoftware\Update\Clients\{30546620-7888-4826-95be-9631ae2eea6e}\* and \ClientState\{30546620-7888-4826-95be-9631ae2eea6e}\* (pv, name values). CORROBORATED: JOE IOC list independently lists Software\PulseSoftware\Update\ClientState\, \ClientStateMedium\, \Clients\, and \Update\ as static strings in the v133.0.6943.205 build.
        "_CLASSESROOT:CLSID\{0EC63402-A762-59E6-8094-42CCD06D5224}"  # SRC: VT CAPE registry_keys_set - LocalServer32\(Default) = "...\updater.exe" --server --service=update-internal
        "_CLASSESROOT:Interface\{1E8CAE0D-DBFB-5F8D-8C4C-631A96B2AE9E}"  # SRC: VT CAPE registry_keys_set - registers as "IUpdaterInternalUser" (ProxyStubClsid32, TypeLib entries)
        "_CLASSESROOT:Interface\{F4F9F399-D63B-5AC7-89F4-7DB385E0BCB8}"  # SRC: VT CAPE registry_keys_set - registers as "IUpdaterInternalCallbackUser" (ProxyStubClsid32, TypeLib entries)
        "_CLASSESROOT:TypeLib\{1E8CAE0D-DBFB-5F8D-8C4C-631A96B2AE9E}"  # SRC: VT CAPE registry_keys_set - "PulseBrowserUpdater TypeLib for IUpdaterInternalUser"
        "_CLASSESROOT:TypeLib\{F4F9F399-D63B-5AC7-89F4-7DB385E0BCB8}"  # SRC: VT CAPE registry_keys_set - "PulseBrowserUpdater TypeLib for IUpdaterInternalCallbackUser"
    )
