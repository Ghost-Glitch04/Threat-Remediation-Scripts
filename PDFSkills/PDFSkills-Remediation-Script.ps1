# ============================================================================ #
# Malware Remediation Framework - PFDSkills
# ============================================================================ #
...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Description = "Trojan-Spy/Adware variant MediaArena (MSIL/.NET). " +
                      "Browser modifier with data exfiltration capabilities. " +
                      "Uses PDF converter lure (PDFSkills). Code-signed by " +
                      "RED ROOT LTD with anti-analysis techniques."
        CVSS = "8.5"
        ThreatIntelSource = "VirusTotal - 38/75 detections"
        AlternateNames = @(
            "BrowserModifier:MSIL/MediaArena",
            "Trojan-Spy.MSIL.MStealer.e",
            "PUP.Optional.MediaArena",
            "Adware.Linkury.161",
            "MSIL/Riskware.MediaArena.Q"
        )
    }
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "D9AFF96830351EB0B8B219729D110822448FE511",  # RED ROOT LTD
            "A42662C6527B001411F9DE4E72A48E7EA8E37E0339035F3248E97DC2F0F69C2D"
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "68 38 E6 70 EA 30 DF A3 29 4E A2 78",
            "6838E670EA30DFA3294EA278"
        )
    ...
    RunKeyPatterns = @(
        ...
        "*\RED ROOT*",
        "*\AppData\*\PDFSkills*",
        "*\ProgramData\*\PDFSkills*"
    )
    
    # Registered applications patterns
    RegisteredAppPatterns = @(
        "*PDFSkills.exe*",
        "*Uninstall.exe*RED*ROOT*",
        "*MediaArena*"
    )
    
    # User-specific paths (exact matches only)
    UserPaths = @(
        "$env:LOCALAPPDATA\PDFSkills",
        "$env:APPDATA\PDFSkills",
        "$env:LOCALAPPDATA\MediaArena",
        "$env:APPDATA\MediaArena",
        "$env:LOCALAPPDATA\RED ROOT",
        "$env:APPDATA\RED ROOT"
    )
    ...
    # Download folder patterns (specific to malware)
    DownloadPatterns = @(
        "PDFSkills*",
        "MediaArena*",
        "*RED*ROOT*"
    ...
    # System-level paths
    SystemPaths = @(
        "$env:ProgramFiles\PDFSkills",
        "$env:ProgramFiles\MediaArena",
        "$env:ProgramFiles\RED ROOT",
        "${env:ProgramFiles(x86)}\PDFSkills",
        "${env:ProgramFiles(x86)}\MediaArena",
        "${env:ProgramFiles(x86)}\RED ROOT",
        "$env:ProgramData\PDFSkills",
        "$env:ProgramData\MediaArena",
        "$env:ProgramData\RED ROOT"
    )
    ...
    RegistryHKLM = @(
        "HKLM:\SOFTWARE\PDFSkills",
        "HKLM:\SOFTWARE\MediaArena",
        "HKLM:\SOFTWARE\RED ROOT",
        "HKLM:\SOFTWARE\WOW6432Node\PDFSkills",
        "HKLM:\SOFTWARE\WOW6432Node\MediaArena",
        "HKLM:\SOFTWARE\WOW6432Node\RED ROOT",
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*PDFSkills*",
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*MediaArena*"
    )
    ...
    RegistryHKUPatterns = @(
        ...
        "Software\MediaArena",
        "Software\RED ROOT",
        "Software\Microsoft\Windows\CurrentVersion\Run\PDFSkills",
        "Software\Microsoft\Windows\CurrentVersion\Run\MediaArena"
    )
    ...
    ApplicationAssociationPatterns = @(
        "*PDFSkills.exe*",
        "*MediaArena*",
        "*_5039a18ba3f7a1f670af1da1f62417e3_*"  # MD5 hash
    )
    ...
    FeatureUsagePatterns = @(
        ...
        "*MediaArena*",
        "*RED ROOT*",
        "*{D53FFDC1-266A-4BF8-AF87-EDCB257ED5B9}*"  # MVID GUID
    )
    
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - File Hash Indicators
    # ----------------------------------------------------------------------------
    
    FileHashes = @{
        MD5    = "5039a18ba3f7a1f670af1da1f62417e3"
        SHA1   = "ed79559d27c6291e1e80ac4bde95a1ae12e92510"
        SHA256 = "6d05e656577192aa65375700db0dd532e8f848a4924411e4b293e2fb2b903721"
    }
}
