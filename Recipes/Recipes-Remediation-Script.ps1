...
$MalwareConfig = @{
    ...
    Metadata = @{
        ...
        Author = "sentinelrshuser"
        ThreatFamily = "Trojan.Toolbar/ALOT"
        FirstSeen = "2008-05-26"
        Severity = "CRITICAL"
        Description = "ALOT Toolbar - Malicious browser toolbar that hijacks Internet Explorer settings, modifies registry keys, and installs unwanted browser extensions. Signed with revoked Alot.com certificate."
        CVSS = "7.5"
        ThreatIntelSource = "VirusTotal - 18/76 detections"
        Aliases = @("rmdymy2.exe", "recipes.exe", "ALOT Toolbar")
        SHA256 = "995927b241901e4c3eaee9def0fb0809c02c85853025e17c4311acc113daef17"
        MD5 = "aec1b0fa94d834a46b2c56913777ffc6"
    }

    Name = "ALOT_Toolbar_Trojan"
    ...
    Certificates = @{
        ...
        MaliciousThumbprints = @(
            "81A59C39236C604B077179635DD19E21B397F244"  # Alot.com code signing cert
        )
        
        # Known malicious certificate serial numbers (PRIORITY 2 - REMOVE)
        MaliciousSerialNumbers = @(
            "1027946633D229F3ECCCF0561C8075",     # Alot.com cert (formatted)
            "10 27 94 66 33 D2 29 F3 EC CC F0 56 1C 80 75"  # Alot.com cert (spaced)
        )
    ...
    UserPaths = @(
        "AppData\Roaming\alot",
        "AppData\Local\alot",
        "AppData\LocalLow\alot"
    )
    ...
    # Download folder patterns (specific to ALOT)
    DownloadPatterns = @(
        "*alot*",
        "rmdymy2.exe",
        "recipes.exe",
    ...
    SystemPaths = @(
        ...
        "C:\Program Files (x86)\alot\bin\alot.dll",
    ...
    RegistryHKLM = @(
        "SOFTWARE\Classes\CLSID\{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}",
        "SOFTWARE\Microsoft\Internet Explorer\Toolbar\{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\alotToolbar",
        "SOFTWARE\Microsoft\Internet Explorer\Explorer Bars\{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}",
        "SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Browser Helper Objects\{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}",
        "SOFTWARE\alot*",
        "SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\alotToolbar"
    )
    ...
    RegistryHKUPatterns = @(
        "Software\alot",
        "Software\alot\*",
        "Software\Microsoft\Internet Explorer\Toolbar\WebBrowser\{5AA2BA46-9913-4DC7-9620-69AB0FA17AE7}",
        "Software\Microsoft\Internet Explorer\Explorer Bars\{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}",
        "Software\Microsoft\Windows\CurrentVersion\Ext\Stats\{5AA2BA46-9913-4DC7-9620-69AB0FA17AE7}"
    )
    ...
    NetworkIndicators = @{
        ...
        URLs = @(
            "http://try.alot.com/tb/recipe/intro_recipe.php",
            "http://update.alot.com/update/update_configs/*",
            "http://rd.alot.com/*",
            "http://widget.alot.com/*",
            "http://rss.alot.com/*"
        )
    }
    ...
    # ----------------------------------------------------------------------------
    # MALWARE CONFIGURATION - Additional IOCs
    # ----------------------------------------------------------------------------
    
    # NSIS installer temp patterns
    NSISPatterns = @(
        "ns*.tmp",
        "nse*.tmp",
        "nsp*.tmp",
    ...
    # Browser Helper Object CLSID
    BHO_CLSID = "{5AA2BA46-9913-4dc7-9620-69AB0FA17AE7}"
    
    # Mutex names created by malware
    MutexNames = @(
        "Shell.CMruPidlList",
        "C__Documents_and_Settings_Administrator_Application_Data_alot_*",
        "C__Users_*_AppData_*_alot_*"
    )
}
